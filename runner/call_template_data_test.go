...
func TestCallTemplateData_ExecuteData(t *testing.T) {
	...
	var tests = []struct {
		...
		{"with template",
			`{"name":"{{.WorkerID}} {{.RequestNumber}} bob {{.FullyQualifiedName}} {{.MethodName}} {{.ServiceName}} {{.InputName}} {{.OutputName}} {{.IsClientStreaming}} {{.IsServerStreaming}} {{Load "../testdata/somefolder/somefile"}} {{List "../testdata/somefolder"}} {{List "../testdata/somefolder" | RandomChoice}}"}`,
			[]byte(`{"name":"worker_id_123 200 bob helloworld.Greeter.SayHello SayHello Greeter HelloRequest HelloReply false false somecontent [../testdata/somefolder/somefile] ../testdata/somefolder/somefile"}`),
			false,
		},
		{"with unknown action",
