...
func TestCallTemplateData_ExecuteData(t *testing.T) {
	...
	var tests = []struct {
		...
		{"with template split range",
			`{"name":"{{Range 0 10 2}} {{Split "1,2,3" ","}} {{Split (Read "../testdata/somefolder/somefile") "con"}}"}`,
			[]byte(`{"name":"[0 2 4 6 8] [1 2 3] [some tent]"}`),
			false,
		},
		{"with template randomslice shuffle",
			`{"name":"{{Range 0 1 1 | RandomSlice}} {{Range 0 1 1 | Shuffle}}"}`,
			[]byte(`{"name":"[0] [0]"}`),
			false,
		},
