...
func TestCallTemplateData_ExecuteData(t *testing.T) {
	...
	var tests = []struct {
		...
		{"with template",
			`{"name":"{{.WorkerID}} {{.RequestNumber}} bob {{.FullyQualifiedName}} {{.MethodName}} {{.ServiceName}} {{.InputName}} {{.OutputName}} {{.IsClientStreaming}} {{.IsServerStreaming}} {{Load "../testdata/list/somefile"}} {{List "../testdata/list"}} {{List "../testdata/list" | RandomChoice}}"}`,
			[]byte(`{"name":"worker_id_123 200 bob helloworld.Greeter.SayHello SayHello Greeter HelloRequest HelloReply false false somecontent [../testdata/listsomefile] ../testdata/listsomefile"}`),
			false,
		},
		{"with unknown action",
